...
function index()
{
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        $content .= <<<php
		<a href="?page=goods&func=one&id={$row['id']}" style="display: inline-block">{$row['name']}<br>
		Цена: {$row['price']}р.<br>
		<img src="$dirImg{$row['url']}" alt="{$row['name']}" style="width: 100px; height: 100px; object-fit: cover;">
		</a>
php;
